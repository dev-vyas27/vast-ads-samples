# VAST Ads Test Feeds (VAST 3.0)

This folder contains sample VAST 3.0 XML files for OTT/CTV ad testing.

## Public XML URLs

- https://dev-vyas27.github.io/vast-ads-samples/linear1.xml
- https://dev-vyas27.github.io/vast-ads-samples/linear2.xml
- https://dev-vyas27.github.io/vast-ads-samples/overlay1.xml
- https://dev-vyas27.github.io/vast-ads-samples/overlay2.xml

## Usage in OTT / Flutter TV app

- **Pre-roll** → use `linear1.xml` or `linear2.xml`
- **Mid-roll** → use `linear1.xml` or `linear2.xml`
- **Overlay** → use `overlay1.xml` or `overlay2.xml`
- **Post-roll** → use `linear1.xml` or `linear2.xml`
EOF
